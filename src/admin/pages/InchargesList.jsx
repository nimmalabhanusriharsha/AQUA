...
import { getIncharges } from '../utils/adminMockData';
...
const InchargesList = () => {
  ...
  return (
    <>
      <AdminHeader title="All Incharges" breadcrumbs={[{ label: 'Organization' }, { label: 'Incharges', active: true }]} />
              ...
              <tbody>
                {filtered.map((item) => (
                  <tr key={item.id} style={{ borderBottom: '1px solid var(--color-border)' }}>
                    ...
                    <td style={{ padding: '16px', fontSize: '14px' }}>{item.agents}</td>
                    ...
                    <td style={{ padding: '16px', textAlign: 'right' }}>
                      <button className="btn-secondary" style={{ padding: '6px 12px', fontSize: '13px', display: 'inline-flex', alignItems: 'center', gap: '6px' }} onClick={() => navigate(`/admin/incharges/${item.id}`)}>
                        <Eye size={16} /> View
                      </button>
                    </td>
                  </tr>
