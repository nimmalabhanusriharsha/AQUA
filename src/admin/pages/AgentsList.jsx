...
import { getAgents } from '../utils/adminMockData';
...
const AgentsList = () => {
  ...
  const filtered = agents.filter(a => 
    ...
    a.incharge.toLowerCase().includes(searchTerm.toLowerCase())
  );

  return (
    <>
      <AdminHeader title="All Agents" breadcrumbs={[{ label: 'Organization' }, { label: 'Agents', active: true }]} />
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '24px' }}>
            <div style={{ display: 'flex', gap: '16px', flex: 1, maxWidth: '500px' }}>
              <div className="input-field" style={{ flex: 1, margin: 0, padding: '8px 12px' }}>
                ...
                <input 
                  type="text" 
                  placeholder="Search by agent, region, or incharge..." 
                  value={searchTerm}
              ...
              <tbody>
                {filtered.map((item) => (
                  <tr key={item.id} style={{ borderBottom: '1px solid var(--color-border)' }}>
                    ...
                    <td style={{ padding: '16px', fontSize: '14px' }}>{item.farmers}</td>
                    ...
                    <td style={{ padding: '16px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                        <div style={{ width: '60px', height: '6px', backgroundColor: '#e2e8f0', borderRadius: '3px', overflow: 'hidden' }}>
                          <div style={{ width: `${item.compliance}%`, height: '100%', backgroundColor: item.compliance >= 90 ? 'var(--status-green)' : 'var(--status-yellow)' }} />
                        </div>
                        <span style={{ fontSize: '13px', fontWeight: 600 }}>{item.compliance}%</span>
                      </div>
                    </td>
                    <td style={{ padding: '16px', textAlign: 'right' }}>
                      <button className="btn-secondary" style={{ padding: '6px 12px', fontSize: '13px', display: 'inline-flex', alignItems: 'center', gap: '6px' }} onClick={() => navigate(`/admin/agents/${item.id}`)}>
                        <Eye size={16} /> View
                      </button>
                    </td>
                  </tr>
